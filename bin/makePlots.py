...
parser.add_argument('--dir',type=str,help='Path to save directory; default=./pdfs/',default='./pdfs/')
...
models = [f for f in glob.iglob(f'{args.mkv}*.mkv')]
...
for tH,tSeq in dataBase:
    for modelGroup,models in modelDict.items():
        fig,axes = plt.subplots(ncols=len(w),nrows=len(models),figsize=(14,6))
        for row,model in enumerate(models):
            lgTbl = np.loadtxt(model)
            k = int(log(lgTbl.size,len(args.a)))
            prefix = os.path.basename(model).split('_')[:2]
            for col,(currWin,currSlide) in enumerate(windowSlide):
        ...
        fig.tight_layout()
        fig.savefig(f'{args.dir}{args.e}_{tH[:5]}_{modelGroup}.pdf')
        plt.close()
    # plotIdx =0
    # for row in ax:
    #     for col in row:
