...
def hsmmViterbi(O,A,E,states,pi,D):
    sampleLen = nbinom.rvs(3.7,.045,loc=20)
    dRange = range(0,500)
    # Initialize list of dictionaries for the current step
    # and ukprev, which tracks the state transitions that maximize the 'viterbi function'
    uk=[{}]
...
def fwd_bkw(observations, states, start_prob, trans_prob, emm_prob, end_st='-'):
    ...
    for i, observation_i in enumerate(observations):
        ...
        for st in states:
            if i == 0:
                ...
                prev_f_sum = logsumexp([f_prev[k]+trans_prob[k][st] for k in states])
    ...
    p_fwd = logsumexp([f_curr[k] + trans_prob[k][end_st] for k in states])
    ...
    for i, observation_i_plus in enumerate(reversed(observations[1:]+[None,])):
        ...
        for st in states:
            if i == 0:
                ...
                b_curr[st] = logsumexp([trans_prob[st][l] + emm_prob[l][observation_i_plus] + b_prev[l] for l in states])

        bkw.insert(0,b_curr)
        b_prev = b_curr

    p_bkw = logsumexp([start_prob[l] + emm_prob[l][observations[0]] + b_curr[l] for l in states])
